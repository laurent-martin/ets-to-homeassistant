...
def fix_objects(obj)
  ...
  obs.each do |obj_id, object|
    # manage in special manner my blinds, identified by "pulse" in address group name
    next unless object[:type].eql?(:sun_protection) && gas[object[:ga].first][:name].end_with?(':Pulse')
  ...
  error = false
  gas.values.select { |ga| ga[:objs].empty? }.each do |ga|
    error = true
